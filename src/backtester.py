from typing import Dict, List
  import pandas as pd
  from src.strategies import ScalpingStrategy, IntradayStrategy, SwingStrategy, PositionStrategy, ArbitrageStrategy, HedgingStrategy
  from src.utils import float_safe
  from src.risk_manager import suggest_position_size

  async def backtest(cfg: Dict, api: Any, symbol: str, start_date: str, end_date: str) -> Dict:
      interval = cfg.get("testing", {}).get("backtest", {}).get("interval", "15m")
      klines = await api.fetch_klines(symbol, interval, limit=1000)
      df = pd.DataFrame(klines, columns=["timestamp", "open", "high", "low", "close", "volume"])
      results = {"trades": [], "win_rate": 0.0, "avg_R": 0.0, "total_profit": 0.0}
      strategies = [
          ScalpingStrategy(cfg, api),
          IntradayStrategy(cfg, api),
          SwingStrategy(cfg, api),
          PositionStrategy(cfg, api),
          ArbitrageStrategy(cfg, api),
          HedgingStrategy(cfg, api)
      ]
      deposit = cfg.get("deposit_usdt", 1000)
      for strat in strategies:
          strat_name = strat.__class__.__name__.replace("Strategy", "").lower()
          for i in range(20, len(df) - 1):
              kl_1m = df.iloc[:i].values.tolist() if interval == "1m" else []
              kl_5m = df.iloc[:i].values.tolist() if interval == "5m" else []
              kl_15m = df.iloc[:i].values.tolist() if interval == "15m" else []
              kl_1h = df.iloc[:i].values.tolist() if interval == "1h" else []
              kl_4h = df.iloc[:i].values.tolist() if interval == "4h" else []
              kl_1d = df.iloc[:i].values.tolist() if interval == "1d" else []
              kl_1w = df.iloc[:i].values.tolist() if interval == "1w" else []
              signal = await strat.generate_signal(None, symbol, kl_1m, kl_5m, kl_15m, kl_1h, kl_4h, kl_1d, kl_1w, [])
              if signal:
                  entry = signal["entry"]
                  atr_pct = signal.get("atr_pct", 0.0)
                  leverage = min(cfg.get("risk", {}).get("max_leverage", 45),
                                max(cfg.get("risk", {}).get("min_leverage", 20),
                                    int(20 + (atr_pct * 1000) * (signal.get("adx", 0.0) / 50))))
                  qty, risk_usdt = suggest_position_size(deposit, entry, atr_pct, 0.005, leverage)
                  risk_dist = atr_pct * entry / 100.0 * cfg.get("strategies", {}).get(strat_name, {}).get("atr_mult_sl", 1.0)
                  sl = entry - risk_dist if signal["side"] == "long" else entry + risk_dist
                  tp1 = entry + 2 * risk_dist if signal["side"] == "long" else entry - 2 * risk_dist
                  for j in range(i, len(df)):
                      price = df["close"].iloc[j]
                      if signal["side"] == "long":
                          if price >= tp1:
                              results["trades"].append({"symbol": symbol, "strategy": strat_name, "R": 2.0})
                              results["total_profit"] += 2 * risk_usdt
                              break
                          elif price <= sl:
                              results["trades"].append({"symbol": symbol, "strategy": strat_name, "R": -1.0})
                              results["total_profit"] -= risk_usdt
                              break
                      else:
                          if price <= tp1:
                              results["trades"].append({"symbol": symbol, "strategy": strat_name, "R": 2.0})
                              results["total_profit"] += 2 * risk_usdt
                              break
                          elif price >= sl:
                              results["trades"].append({"symbol": symbol, "strategy": strat_name, "R": -1.0})
                              results["total_profit"] -= risk_usdt
                              break
      results["win_rate"] = len([t for t in results["trades"] if t["R"] > 0]) / len(results["trades"]) if results["trades"] else 0.0
      results["avg_R"] = sum([t["R"] for t in results["trades"]]) / len(results["trades"]) if results["trades"] else 0.0
      return results
